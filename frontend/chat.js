...
window.addEventListener('DOMContentLoaded', async()=>{
    try{
        const allG = await axios.get('http://localhost:3000/message/allGroup', { headers: {'Authorization': token}} );
        ...
        arrG.forEach(ele=>{
            ...
            li.addEventListener('click',async()=>{
                ...
                const allU = await axios.get(`http://localhost:3000/message/allUsers?gId=${activeGroup}`, { headers: {'Authorization': token}});
...
cGroupForm.addEventListener('submit', (e)=>{
    ...
    axios.post('http://localhost:3000/message/createGroup', gNameObj, { headers: {'Authorization': token}})
    .then((response) => {
...
inviteBtn.addEventListener('click', async()=>{
    ...
    const inviteLink = await axios.get(`http://localhost:3000/message/getInvite?gId=${activeGroup}`, { headers: {'Authorization': token}});
...
joinGroupFrom.addEventListener('submit',async(e)=>{
    ...
    const joinRes = await axios.get(`http:localhost:3000/message/joinGroup?gId=${id}`, { headers: {'Authorization': token}});
...
addUserForm.addEventListener('submit',async (e)=>{
    ...
    const addUserRes = await axios.get(`http:localhost:3000/message/addUser?by=${addUserBy}&value=${addUserValue}&gId=${activeGroup}`, { headers: {'Authorization': token}});
    ...
    if(isAd){
        ...
        if(presentUId !== ele.userId){
            ...
            remBtn.addEventListener('click', async () => {
                ...
                const removed = await axios.get(`http:localhost:3000/message/removeU?id=${ele.userId}&gId=${activeGroup}`);
                window.location.reload();
            })
        ...
        if(ele.isAdmin){
            ...
            makeAdmin.addEventListener('click', async ()=>{
                ...
                const admined = await axios.get(`http://localhost:3000/message/makeA?id=${ele.userId}&gId=${activeGroup}`);
                window.location.reload();
            })
